...
Manages four collections (docs, issues, PRs, code) in a self-hosted Qdrant
instance.  All collections use HNSW indexes with cosine distance and
payload indexes on filterable fields.

Design decisions (see ADR-001 and ADR-003):
- Four separate collections — one per source type — enable independent
  HNSW tuning and clean schema evolution.
- HNSW parameters (m=16, ef_construct=100) balance recall and index build
  speed for documentation-scale workloads.
- Payload indexes on ``source_type``, ``repo``, ``state``, and ``language``
  allow efficient filtered searches without full collection scans.
...
class QdrantVectorStore(BaseVectorStore):
    """Qdrant-backed vector store for the Gardener AI MCP project.

    Manages creation and access to the four canonical collections.  All
    HNSW and index configuration is applied at collection-creation time;
    subsequent calls to ``ensure_collection`` on an already-existing
    collection are no-ops.

    Attributes:
        COLLECTIONS: The four collection names used across the project.
    ...
    async def ensure_collection(
        ...
        ``_INDEXED_PAYLOAD_FIELDS`` (``source_type``, ``repo``,
        ``state``, ``language``).
    ...
    async def search(
        ...
        Args:
            ...
            filters: Optional exact-match payload filters.  E.g.
                ``{"source_type": "docs", "repo": "gardener/gardener"}``.
