...
async def _check() -> None:
    ...
    if any_empty:
        ...
        print(
            ...
            f"  Or:   uv run python scripts/ingest_docs.py --collections {aliases}",
            file=sys.stderr,
        )
        sys.exit(1)
...
async def _ingest_collection(
    ...
    """Ingest a single documentation collection end-to-end.

    Args:
        collection: One of the ``_ALL_COLLECTIONS`` names.
    ...
    repo_slug = _repo_for(collection, settings)
    content_type = _content_type_for(collection)

    print(f"\n{'─' * 60}", flush=True)
    print(f"  Collection: {collection}  (repo={repo_slug})", flush=True)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Ingest SCI documentation into Qdrant.",
